<template>
  <div class="asistente">
    <div class="page-header">
      ...
      <p class="subtitle">
        Consultá sobre salud, síntomas y cuidados de tu mascota
      </p>
    </div>
    ...
    <div class="chat-wrapper">
      <div class="messages" ref="messagesRef">
        <div v-if="mensajes.length === 0" class="empty-chat">
          ...
          <p>
            Preguntame sobre síntomas, alimentación, cuidados o cualquier duda
            sobre tu mascota.
          </p>
      ...
      <div class="input-area">
        <input
          ...
          :disabled="cargando"
        />
        <button @click="enviar" :disabled="cargando || !input.trim()">
          Enviar
        </button>
      </div>
    </div>
...
async function enviar() {
  ...
  try {
    ...
    mensajes.value.push({
      role: "model",
      text: "Error al conectar con el asistente. Verificá tu API key.",
    });
  } finally {
...
.bubble :deep(p) { margin: 0 0 0.5rem 0; }
.bubble :deep(p:last-child) { margin-bottom: 0; }
.bubble :deep(ul), .bubble :deep(ol) { margin: 0.5rem 0; padding-left: 1.25rem; }
.bubble :deep(li) { margin-bottom: 0.25rem; }
.bubble :deep(strong) { font-weight: 700; }
.bubble :deep(code) { background: rgba(0,0,0,0.08); padding: 0.1rem 0.35rem; border-radius: 4px; font-size: 0.85em; }
.bubble :deep(h1), .bubble :deep(h2), .bubble :deep(h3) { margin: 0.5rem 0 0.25rem 0; font-size: 1em; font-weight: 700; }
